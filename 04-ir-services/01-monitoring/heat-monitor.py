...
# path for the ir-codes
ir_codes_path = os.getenv('ir_codes_path', '../ir-codes')
ir_command_default = 'bto_advanced_USBIR_cmd -d `cat {}'.format(ir_codes_path)

def turn_on_ac():
    """Turn on the ac to 24c medium.
    """
    global last_ac_status
    global ir_command_default
    med_blow_command = '{}/ac/24c-med-blower.txt`'.format(ir_command_default)
    os.system(med_blow_command)
    last_ac_status = 1
...
def turn_off_ac():
    ...
    global last_ac_status
    global ir_command_default
    off_blow_command = '{}/ac/off.txt`'.format(ir_command_default)
    os.system(off_blow_command)
    last_ac_status = 0
